...
export class AbstractCLI {
    ...
    spawnCommand(args, silent = false) {
        console.log(`
`, chalk.yellow.bold('☀ @leanup/cli'), `(v${this.version})`, chalk.italic.gray('execute the following pure command ...'), `
...
`);
        return spawn('npx', args, {
            ...
            if (code > 0) {
                console.log(`
`, chalk.red.bold('✘ @leanup/cli'), chalk.italic.gray('command execution ended with error'));
                process.exit(code);
            }
            console.log(`
`, chalk.green.bold('✔ @leanup/cli'), chalk.italic.gray('command execution successfully completed'));
        });
    }
